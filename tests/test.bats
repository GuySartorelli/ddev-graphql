setup() {
  ...
  export TESTDIR=~/tmp/test-graphql
  mkdir -p $TESTDIR
  export PROJNAME=test-graphql
...
@test "install from release" {
  set -eu -o pipefail
  cd ${TESTDIR} || ( printf "unable to cd to ${TESTDIR}\n" && exit 1 )
  echo "# ddev get machine-rc/ddev-graphql with project ${PROJNAME} in ${TESTDIR} ($(pwd))" >&3
  ddev get machine-rc/ddev-graphql
